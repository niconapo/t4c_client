...
void CWeather::DrawRain2(CV2Sprite *pR1,CV2Sprite *pR1M,
                         CV2Sprite *pR2,CV2Sprite *pR2M,
                         CV2Sprite *pR3,CV2Sprite *pR3M,
                         CV2Sprite *pVSFSpriteL,V2SPRITEFX *fx)
{
   //InitSound(); // Ophise Add'
   static BOOL bShowLight1 = FALSE;
   static BOOL bShowLight2 = FALSE;
   ...
   if((dwCnt == 0 || dwCnt%2) || m_dwRainIntensity>3)
   {
      for(UINT i=0;i<dwNbrGouttes;i++)
      {
         ...
         int iRandG = rand()%3;
         if(iRandG == 0)
            pR1->DrawSpriteN(m_pRainX[i], m_pRainY[i],fx,pR1M);
         else if(iRandG == 1)
            pR2->DrawSpriteN(m_pRainX[i], m_pRainY[i],fx,pR2M);
         else if(iRandG == 2)
            pR3->DrawSpriteN(m_pRainX[i], m_pRainY[i],fx,pR3M);
      }
   }
   else 
   {
