...
LightMap::LightMap(void) 
{
   ...
   m_uiGrayStatus    = 0;
   m_uiGrayStatusCnt = 0;
   Custom.m_bDisplayInGrayScale = false;
}
...
do_next_block:
         ...
         movdqu      xmm2, xmm0
         movdqu      xmm5, xmm3
         movdqu      xmm6, xmm0           // Copy of 4 pixels light
         ...
         movdqa      xmm0, xmm6           // Restore 4 pixels light
         movdqa      xmm1, xmm6           // Restore 4 pixels light
         movdqa      xmm2, xmm6           // Restore 4 pixels light
         ...
         pand        xmm1, GREENMASK_555   // no need if 24bpp light map, ...
         pand        xmm2, BLUEMASK_555    // no need if 24bpp light map, ...
         punpckhwd   xmm0, xmm0            // Shuffle first 2 light pixels
         punpckhwd   xmm1, xmm1            // Shuffle first 2 light pixels
         punpckhwd   xmm2, xmm2            // Shuffle first 2 light pixels
         ...
         por         xmm3, xmm4
         add         esi, 16      // Advance to the next height pixels.

         movdqu	     [edi], xmm3

         add         edx, 16      // Advance to the next height pixels.
         add         edi, 16      // Advance to the next height pixels.
...
done_pixels_blocks:
      ...
      // Increment pointers to remainder positions
      lpwSource += NumBlocks * 8 * 2;
      lpwDest   += NumBlocks * 8 * 2;
      lpwLight  += NumBlocks * 8;
