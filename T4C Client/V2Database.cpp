...
#define V2NMSDATABASE_INDEX  "Game Files\\V2NMSDataI.did"
#define V2NMSDATABASE_DATA   "Game Files\\V2NMSData" //need to add index and .dda
...
{
   m_pDataBuffer = new MemMapFile[NB_FILEST];
   for(int i=0;i<NB_FILEST;i++)
   {
...
void CV2Database::Release(void) 
{
   ...
   for( itm = idInfo.begin(); itm != idInfo.end(); itm++ )
   {
      ...
      ULONG UlVal = (ULONG)ii->pstrNom;
      if(UlVal != 0xfeeefeee)
         int caca = 1;

      if (ii->pstrNom && _CrtIsValidHeapPointer(ii->pstrNom))
      {
         delete []ii->pstrNom;
         ii->pstrNom = NULL;
      }
      if (ii->lpSpriteInfo && _CrtIsValidHeapPointer(ii->lpSpriteInfo)) 
      {
...
{ 
   ...
   FILE *pFile = NULL;
   fopen_s(&pFile,V2DATABASE_INDEX,"rb");
   if(!pFile)
      return ;
   ...
   //Load NMS Index...
   FILE *pFileNMS = NULL;
   fopen_s(&pFileNMS,V2NMSDATABASE_INDEX,"rb");
   if(pFileNMS)
   {
      fread(strChksumMd5    ,1,16,pFileNMS);
      fread(&ulTailleUnpack ,4,1 ,pFileNMS);
      fread(&ulTaillePack   ,4,1 ,pFileNMS);
      fread(pstrMD5Part2    ,1,17,pFileNMS);

      BYTE *pchDataNMS = new unsigned char[ulTailleUnpack];
      BYTE *pCompressed= new unsigned char[ulTaillePack];
   
      fread(pCompressed,1,ulTaillePack,pFileNMS);
      fread(&uchChksum ,1,1,pFileNMS);
      fclose(pFileNMS);
      ...
      UINT dwNbrIndex = (ulTailleUnpack/sizeof(sHeaderFile));
      BYTE *pchDataTmp = pchDataNMS;
      for(i=0;i<dwNbrIndex;i++)
      {
         sHeaderFile newHeaderTmp;
         pchDataTmp = pchDataNMS+(i*sizeof(sHeaderFile));
         memcpy(&newHeaderTmp,pchDataTmp,sizeof(sHeaderFile));
         ...
         memcpy(ii->pstrNom,newHeaderTmp.strNom,ii->chNbrNom+1);
         ii->dwFileOffset    = newHeaderTmp.dwFileOffset;
         ii->dwDataFileIndex = newHeaderTmp.dwDataFileIndex+NB_FILESORI;
   ...
   // load les file ORI
   int iCnt = 0;
   for (i=0;i<NB_FILESORI;i++)
   {
      sprintf_s(strName,128,"%s%02d.dda",V2DATABASE_DATA,i);
      m_bDataBufferLoaded[iCnt] = m_pDataBuffer[iCnt].Create(strName,false);
      iCnt++;
   }
   for (i=0;i<NB_FILESNMS;i++)
   {
      sprintf_s(strName,128,"%s%02d.dda",V2NMSDATABASE_DATA,i);
      m_bDataBufferLoaded[iCnt] = m_pDataBuffer[iCnt].Create(strName,false);
      iCnt++;
   }
   ...
   else  
   {
      *lplpSpriteInfo = new V2SPRITEINFO;
      ...
      switch ((*lplpSpriteInfo)->Header.dwCompType) 
      {
         ...
         break;
         case COMP_BFZ:
            LoadSprite_BFZ(lplpSpriteInfo, ii,lpszID,lpPal,iPalIndex,dwOffsetData);
         break;
...
void CV2Database::LoadSprite_NCK(LPV2SPRITEINFO *lplpSpriteInfo,LPDATAHEADER ii,LPCTSTR lpszID,LPBYTE lpPal,int iPalIndex, int dwDataCompIndex)
{
   ...
   if(iPalIndex != 0 || (*lplpSpriteInfo)->Header.ushTransparency ==6 || (*lplpSpriteInfo)->Header.ushTransparency ==11 || (*lplpSpriteInfo)->Header.ushTransparency ==12|| (*lplpSpriteInfo)->Header.ushTransparency ==10)
   {
      int i,x,y,nb_pix;
      BYTE	*pt_pack;
...
VOID CV2Database::LoadSprite_ZIP ( LPV2SPRITEINFO *lplpSpriteInfo,LPDATAHEADER ii,LPCTSTR lpszID,LPBYTE lpPal,int iPalIndex, int dwDataCompIndex)
{
   ...
   if(iPalIndex != 0 || (*lplpSpriteInfo)->Header.ushTransparency ==6|| (*lplpSpriteInfo)->Header.ushTransparency ==11 || (*lplpSpriteInfo)->Header.ushTransparency ==12 || (*lplpSpriteInfo)->Header.ushTransparency ==10)
   {
      ...
      if((*lplpSpriteInfo)->Header.ushTransparency == 10)
      {
         (*lplpSpriteInfo)->Data.lpbChunck = new BYTE[dwTailleUnc];
         memcpy((*lplpSpriteInfo)->Data.lpbChunck,pchUnpack,dwTailleUnc);
      }
...
// int        iPalIndex
//////////////////////////////////////////////////////////////////////////////////////////
VOID CV2Database::LoadSprite_BFZ ( LPV2SPRITEINFO *lplpSpriteInfo,LPDATAHEADER ii,LPCTSTR lpszID,LPBYTE lpPal,int iPalIndex, int dwDataCompIndex)
{
   if((*lplpSpriteInfo)->Header.dwWidth >180 || (*lplpSpriteInfo)->Header.dwHeight >180)//:CR_NMNMNM Valid
   {
      BYTE *pDdata = new BYTE[(*lplpSpriteInfo)->Header.dwDataPack];
   ...
   //if(iPalIndex != 0 || (*lplpSpriteInfo)->Header.ushTransparency ==6|| (*lplpSpriteInfo)->Header.ushTransparency ==11 || (*lplpSpriteInfo)->Header.ushTransparency ==12 || (*lplpSpriteInfo)->Header.ushTransparency ==10)
   {
      unsigned long dwTailleUnc = *(unsigned int *)&(*lplpSpriteInfo)->Data.lpbChunck[0];
      BYTE *pData = (*lplpSpriteInfo)->Data.lpbChunck+4;
      BYTE *pchUnpack = new BYTE[(*lplpSpriteInfo)->Header.dwDataUnpack+4];
      

      char strNom[256];
      sprintf_s(strNom,256,"%s+NMS666",lpszID);
      
      BlowFishEnc *crypt = new BlowFishEnc(strNom);
      dwTailleUnc = crypt->decryptStream((char*)pData, (*lplpSpriteInfo)->Header.dwDataUnpack, (char*)pchUnpack);
      delete crypt;
...
void CV2Database::LoadSprite_DD( LPV2SPRITEINFO *lplpSpriteInfo,LPDATAHEADER ii, int dwDataCompIndex)
{
  ...
  
   {
	   DDSURFACEDESC2 Surface; 
		ZeroMemory(&Surface, sizeof(DDSURFACEDESC2)); 
		...
		DXDErrorHandle(lpDXDirectDraw->CreateSurfaceNM(&Surface, &(*lplpSpriteInfo)->Data.lpDDSurface, NULL), "Cheap Ass"); 
   ...
   if(!lpbDest)
   {
      (*lplpSpriteInfo)->Data.lpDDSurface->Unlock(NULL);
      return;
   }
