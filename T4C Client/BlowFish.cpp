...
bool BlowFishEnc::ReadEncryptedFile(char* _szFIn, char **ppDecryptedData,unsigned int &iBuffSize, char **pStrVersion_, int &iTextSize_)
{
   bool bOK = false;

   if(*ppDecryptedData)
      delete [](*ppDecryptedData);
   
   unsigned int uiTextSizeU   = 0;
   unsigned int uiTextSizeC   = 0;
   unsigned int uiFileSize    = 0;
   unsigned int uiFileSizeCnt = 0;
   FILE	*hReadFile=NULL;
   fopen_s(&hReadFile,_szFIn,"rb");
   if(hReadFile)
   {
      if(*pStrVersion_)
         delete [](*pStrVersion_);
      
      //procvess with buffser size...
      const int iBufferSize = 1024;
      
      DWORD  dwReadSize;
      DWORD  dwEncReturn;
      
      char chReadBuffer[iBufferSize];
      char chOutBuffer [iBufferSize];
      
      //read file length...
      fread(&uiFileSize, sizeof(unsigned int), 1, hReadFile);
      //read txt info...
      fread(&uiTextSizeU, sizeof(unsigned int), 1, hReadFile);
      fread(&uiTextSizeC, sizeof(unsigned int), 1, hReadFile);
      
      if(uiTextSizeC < uiTextSizeU || uiTextSizeC-uiTextSizeU > 10)
      {
         fclose(hReadFile);
         return false;
      }
      
      //decode file version info...
      (*pStrVersion_) = new char[uiTextSizeU+1];
      memset((*pStrVersion_),0x00,uiTextSizeU+1);
      
      char *pchOutBuffer = new char[uiTextSizeC];
      memset(pchOutBuffer,0x00,uiTextSizeC);
      
      char *pchOutBuffer2 = new char[uiTextSizeC];
      memset(pchOutBuffer2,0x00,uiTextSizeC);
      
      fread(pchOutBuffer, sizeof(char), uiTextSizeC, hReadFile);
      dwEncReturn = decryptStream(pchOutBuffer, uiTextSizeC, pchOutBuffer2);
      
      
      memcpy((*pStrVersion_),pchOutBuffer2,uiTextSizeU);
      
      iTextSize_ = uiTextSizeU;
      ...
      if(pchOutBuffer2)
         delete []pchOutBuffer2;
         pchOutBuffer2 = NULL;


      //Create Decrypt Buffer
      (*ppDecryptedData) = new char[uiFileSize];
      iBuffSize          = uiFileSize;

      char *pDestbBuf    = (*ppDecryptedData);
      ...
      {
         memset(chOutBuffer,0x00,iBufferSize);
         dwReadSize  = fread(chReadBuffer, sizeof(char), iBufferSize, hReadFile);
         dwEncReturn = decryptStream(chReadBuffer, dwReadSize, chOutBuffer);
         
         if(feof(hReadFile))
         {
            dwEncReturn = uiFileSize-uiFileSizeCnt;
         }
         memcpy(pDestbBuf,chOutBuffer,dwEncReturn);
         uiFileSizeCnt += dwReadSize;
         pDestbBuf     += dwReadSize;
      }
