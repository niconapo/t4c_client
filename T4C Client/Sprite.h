...
class Sprite {
	private:
		LPDIRECTDRAWSURFACE7  lpSpriteSurface; // SpriteSurface.
	...
	public:
      ...
      Sprite(void);					           // Class Constructor Default
		...
		void LoadSprite(LPDIRECTDRAWSURFACE7 lpImageSurface, int xPos, int yPos, short width, short height,
			             bool ftransparent = FALSE, int transcolor = 0); // Load a Sprite from an Image
		void SaveSprite(char *SpriteID);   // Save a Sprite
		void DrawSprite(int xPos, int yPos, RECT *Clip = NULL, RECT *OutBound = NULL, RECT *Wrap = NULL, LPDIRECTDRAWSURFACE7 ToBlit = lpDXDirectDraw->GetBackBufferDirect(), char FX = 0, signed char Format = 0, BOOL ParamDither = FALSE);  // Draw a Sprite
      ...
      LPDIRECTDRAWSURFACE7 GetSurface(void);
		RECT *				  GetRect(void);
      ...
      VOID                ReleaseSurfaceMemory(LPWORD);
		...
		bool Mark;
   protected:
      HRESULT TransAlphaImprovedOldSpr(LPDIRECTDRAWSURFACE7 src, LPDIRECTDRAWSURFACE7 dest,LONG lDestX, LONG lDestY,LONG lDestW,LONG lDestH, RECT srcRect, WORD ALPHA, DWORD ColorKey, WORD BPP);
};
