...
{ 
   ...
   // Si c pas deja loader, on load.
      strcpy_s(SoundName,100, lpszFile);
...
{ 
   ...
   { 
      int iVol = v*5;
      int dwVal2 = (iVol/2)+50;
      long dwVolumeTmp = (dwVal2 * 10000) / 100;
      int dwVolume    = (10000 - dwVolumeTmp)*-1;

     
      sbBuffer.SetVolume(dwVolume); 
...
VOID T3VSBSound::CopyWave( T3VSBSound *vsb ) 
{
   if (ug_T3SoundManager->fFlag & TS_SOUNDDRIVER) { 
      ...
      strcpy_s(SoundName,100, "Copy of ");
      strcat_s(SoundName,100, vsb->SoundName);
