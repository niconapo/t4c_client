...
{
    text = FontManager::GetInstance()->GetSmallFont();
...
//////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    if( cursorEnabled && cursor != NULL ){
        ...
        if( cursorState && (cursorEnabledF ||proceed) ){
            // Display cursor
            cursor->Draw( vsfFX );
        }
...
{
   char strTmp[2048];
   sprintf_s(strTmp,2048,"%s",currentText.c_str());
   strTmp[cursorPos] = 0x00;
   
   text->SetText(strTmp, g_DefColorD, 0, 0, 0 );

   textOffset = 0;
   // If the text is bigger than the control
   if( text->GetWidthTO() > GetPos().Width()  && cursorPos >0)
   {
      // If the cursor is at the end of the text.
      if( cursorPos >= strlen(strTmp)/*currentText.size()*/ )
      {
         // Put cursor at end of text.            
         textOffset = text->GetWidthTO() - GetPos().Width();
      }
   }
   text->SetText( currentText.c_str(), g_DefColorD, 0, 0, 0 );
   ...
   if( cursorPos > 0 )
   {
      ...
      textWidth = text->GetWidthTO( cursorText.c_str() );
   }
