...
public:
    bool operator()( const std::string &r, const std::string &l ) const{
        return( _stricmp( r.c_str(), l.c_str() ) < 0 );
    }
};    
...
public:    
    ...
    virtual void SetSpriteId( std::string newSpriteId ,bool bMask = false,std::string newSpriteIdMask = "");
...
private:       
    ...
    std::string  spriteId;
    std::string  spriteIdMask;
    CV2Sprite    vsfSprite;
    CV2Sprite    vsfSpriteMask;
    TextObject  *helpText;
    bool         hiddenState;
...
inline void SetHelpId( GameUI *gameUI, DWORD helpId ){
    gameUI->SetHelpText( FontManager::GetInstance()->GetSmallFont(g_LocalString[ helpId ] ) );
}
inline void SetHelpId( GameUI &gameUI, DWORD helpId ){
