...
const int g_maxHelpText = 400; //250
    ...
    {    
        {
            ...
            if( !spriteId.empty())
            {
               ...
               {
                  if(m_bUseMask)
                     vsfSprite.DrawSpriteN( framePos.x, framePos.y, vsfFX,&vsfSpriteMask);
                  else
                     vsfSprite.DrawSpriteN( framePos.x, framePos.y, vsfFX);

               }
               else
                  vsfSprite.DrawSpriteNSemiTrans( framePos.x, framePos.y, vsfFX ,false,g_Global.GetAlphaValue()); //CV:GuiAlpha
            }
            ...
            for( i = childList.rbegin(); i != childList.rend(); i++ )
            {
               ...
               {
                  if( (*i)->IsShown() )//NMNMNM_DeathNMNMNM
                  {
...
void GameUI::Show(bool putForeground)
{
    // If this box has a spriteId.
    if( !spriteId.empty() )
        vsfSprite.LoadSprite( spriteId.c_str() );// Load the UI's graphic.

    if( !spriteIdMask.empty() )
        vsfSpriteMask.LoadSprite( spriteIdMask.c_str() );// Load the UI's graphic.
...
//////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    if( helpText != NULL )
    {
        int maxX = g_maxHelpText;
        if( helpDisplayX + maxX >= g_Global.GetScreenW() )
        {
            maxX = g_Global.GetScreenW() - helpDisplayX;
        }

        if(helpDisplayX+g_maxHelpText > g_Global.GetScreenW())
        {
           if( helpDisplayX+helpText->GetWidthTO() > g_Global.GetScreenW() )
           {
              int minX  = g_Global.GetScreenW()-g_maxHelpText;
              if(helpText->GetWidthTO() <g_maxHelpText)
                 minX  = g_Global.GetScreenW()-helpText->GetWidthTO();
