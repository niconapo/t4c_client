...
/*
typedef struct _sQuestBookList
{
   ...
   std::string   strMessage;             // Init message
   //std::vector <std::string> aStepMsg;   // la liste des message pour chaque step
}sQuestBookList;
*/

class V3_QuestBookP1 : public BoxUI  
{
public:	
	virtual ~V3_QuestBookP1();


    static V3_QuestBookP1 *GetInstance();
    ...
    void ClearAllList();
    void UpdateQuestList   (USHORT ushUniqueID,USHORT ushLevel,BYTE chStatus,char *pstrName);
    void UpdateQuestMessage(USHORT ushUniqueID,char *pMsg);
    void UpdateDisplayList();
    int GetListSize(){return m_vQuestListL.size();}
    

    class sQuestBookList
    {
       public:
          static bool compareMyDataPredicate(sQuestBookList lhs, sQuestBookList rhs) 
          { 
             std::string s1,s2;
...
private:
   // List containers.
   std::vector <sQuestBookList>  m_vQuestList;
   std::vector <sQuestBookList>  m_vQuestListL;
