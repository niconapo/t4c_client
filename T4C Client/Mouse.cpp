...
extern BOOL         g_bFirstLoadComplete;
...
UINT MouseDevice::Buttons[4]= { DM_NONE, DM_NONE, DM_NONE, DM_NONE };;
UINT MouseDevice::ButtonsState[4]= { DM_NONE, DM_NONE, DM_NONE, DM_NONE };;
DWORD MouseDevice::Click[4]= { 0, 0, 0, 0 };
int MouseDevice::xClick[4]= { 0, 0, 0, 0 };;
int MouseDevice::yClick[4]= { 0, 0, 0, 0 };;
DWORD MouseDevice::ButtonDown[4]= { 0, 0, 0, 0 };
...
bool MouseDevice::IsLockNextEvent( int i) 
{
...
//**************************************************************************************//
{
	...
	for (int i = 0; i < 4; i++) 
   {
      ...
      {
         if (timeGetTime() - ButtonDown[i] > 250) //NMNMNM_Drag changer de 250 a 10
         {
...
UINT WINAPI QueryMouseEventThread(LPVOID pParam) 
{
   ...
   {
      char Temp[1024];
      sprintf_s(Temp,1024, g_LocalString[155], 2, hresult);
      g_App.SetError(1, Temp);
      g_App.Close(Temp,true);
   }
   Sleep(15);
   ...
   {
      char Temp[1024];
      sprintf_s(Temp,1024, g_LocalString[155], 3, hresult);
      g_App.SetError(1, Temp);
      g_App.Close(Temp,true);
   }
   Sleep(15);
   ...
   {
      char Temp[1024];
      sprintf_s(Temp,1024, g_LocalString[155], 5, hresult);
      g_App.SetError(1, Temp);
      g_App.Close(Temp,true);
   }
   Sleep(15);
   ...
   {
      char Temp[1024];
      sprintf_s(Temp,1024, g_LocalString[155], 4, hresult);
      g_App.SetError(1, Temp);
      g_App.Close(Temp,true);
   }
   ...
   {
      char Temp[1024];
      sprintf_s(Temp,1024, g_LocalString[155], 6, hresult);
      g_App.SetError(1, Temp);
      g_App.Close(Temp,true);
   }
   Sleep(15);
    ...
    do 
    {
       ...
       if (Custom.gWindowed)
       {
          ...
          {
             if( pos.x > poswin.left && pos.x < poswin.right && pos.y > poswin.top && pos.y < poswin.bottom)
                InScreen=true;
             else 
                InScreen=false;
          }
       ...
       else if (!g_App.isFocus())
          InScreen=false;
       
       
       if (InScreen || g_Global.GetDraggingDialog()) 
       {
          // DIMOUSESTATE
          lpDIMouseDevice->GetDeviceState(sizeof(Status), &Status);        
          ...
          if (MouseDevice::xPos  > g_Global.GetScreenW())
             MouseDevice::xPos = g_Global.GetScreenW();
          else if (MouseDevice::xPos  < 0)
             MouseDevice::xPos = 0;
             
          
          if (MouseDevice::yPos  > g_Global.GetScreenH())
             MouseDevice::yPos = g_Global.GetScreenH();
          else if (MouseDevice::yPos  < 0)
             MouseDevice::yPos = 0;
             ...
             if( buttonStatus[ 1 ] != Status.rgbButtons[ 1 ] )
             {
                ...
                else if( MouseDevice::ButtonsState[ 0 ] == DM_DRAG )
                {
                   //if( GetTickCount() >= lastDragTime )
                   {
                      ...
                      lastDragTime = GetTickCount() + 50; // ms //NMNMNM_Drag  Time Changer de 50 a 10
                   }
                }
...
void CMouseCursor::SetCustomCursor(CV2Sprite *cc, int x, int y, bool corr ,bool bGUIForced,CV2Sprite *ccA) 
{
   ...
   if(cc)
   {
      if(vsfCursor != cc)
      {
         if(!m_bGUIForced) //on backup que les curseur autre que ceux forced GUI
         {
            vsfCursorOld  = vsfCursor;
            vsfCursorOldA = vsfCursorA;
         }
         vsfCursor    = cc;
         vsfCursorA   = ccA;
         iXCor = x;
         iYCor = y; 
         correction = corr;
         m_bGUIForced = bGUIForced;
      }
   }
   else if(m_bGUIForced)
   {
      //restore previous
      vsfCursor    = vsfCursorOld;
      vsfCursorA   = vsfCursorOldA;
      vsfCursorOld = NULL;
      vsfCursorOldA= NULL;
      iXCor = x;
      iYCor = y; 
...
CV2Sprite * CMouseCursor::GetCustomCursor()
{
   return vsfCursor;
}

CV2Sprite * CMouseCursor::GetCustomCursorA()
{
   return vsfCursorA;
}

bool CMouseCursor::GetGUIForced()
{
   return m_bGUIForced;
}
...
void CMouseCursor::DrawCursor(int x, int y) 
{
    ...
    if (Force || vsfCursor) 
    {
        ...
        if (Force != 9+NBCURSOR && !vsfCursor) // Determine the exact forced cursor to use.
        {
           //convertie xpos et ypos en base 1024*768...
           int dwMouseDistance = 512*3;    //1204/2
           xPos = xPos*1024/g_Global.GetScreenW();
           yPos = yPos*768/g_Global.GetScreenH();
           ...
           int b = yPos*4;
            ...
            if (b > dwMouseDistance) 
            {
                if (a > dwMouseDistance) 
                {
                    ...
                    b -= dwMouseDistance;
                    if (a > b*2)
                        Force = 3+NBCURSOR; //Est
                    else if (b > a*2)
                        Force = 5+NBCURSOR; //Sud
                    else 
                        Force = 4+NBCURSOR; // Sud Est
                } 
                else 
                {
                    b -= dwMouseDistance;
                    a = dwMouseDistance - a;
                    if (a > b*2)
                        Force = 7+NBCURSOR; //West
                    else if (b > a*2)
                        Force = 5+NBCURSOR; //Sud
                    else
                        Force = 6+NBCURSOR; //sud west
                }
            } 
            else 
            {
                if (a > dwMouseDistance) 
                {
                    ...
                    if (a > b*2)
                        ...
                        Force = 1+NBCURSOR; //north
                    else
                        Force = 2+NBCURSOR; //north est
                } else 
                {
                    a = dwMouseDistance - a;
                    b = dwMouseDistance - b;
        ...
        if (vsfCursor) 
        {
            vsfCursor->DrawSpriteN(x+iXCor, y+iYCor, &fx,vsfCursorA);
        } 
        else 
        {
            lplpCursor[Force]->DrawSpriteN( x + iXCor, y+iYCor, &fx,lplpCursorA[Force] );
        }
        LeaveCriticalSection(&csInternal);
        return;
    } 
    else 
    {
        ...
        if(g_bFirstLoadComplete && g_bSecondLoadComplete)
        {
            // Release the cursor looking for the new UI ownership.
            LeaveCriticalSection(&csInternal); //NMNMNM a valider le remove du lock...
            mouseOwned = RootBoxUI::GetInstance()->IsMouseOwned( MousePos( x, y ) );
            EnterCriticalSection(&csInternal);
        }
        ...
        if( mouseOwned )
        {
            ...
            lplpCursor[0]->DrawSpriteN( x + iXCor, y+iYCor, &fx ,lplpCursorA[0]);            
        }
        else
        {
            ...
            if (Grid(((xPos+48-16)/xGridPixel),(yPos-8)/yGridPixel)) 
            {
               ...
               lplpCursor[dwType]->DrawSpriteN( x + iXCor, y+iYCor, &fx ,lplpCursorA[dwType]);

            } 
            else 
            {
                int iPos = Grid(((xPos+48-16)/xGridPixel),(yPos-8)/yGridPixel);
                lplpCursor[iPos]->DrawSpriteN( x + iXCor, y+iYCor, &fx ,lplpCursorA[iPos]);
            }
        }
    }
