...
void LayeredTile::Layer::SetIntensity(int x, int y, unsigned char ucVal)
{
   //return m_Intensity.At<unsigned char>(x, y, 0);
   m_Intensity[y][x] = ucVal;
}

int LayeredTile::Layer::Save( std::ofstream& rOutputStream ) const 
{
   for (unsigned int j = 0; j < LayerHeight; ++j)
   {
      for (unsigned int i = 0; i < LayerWidth; ++i)
...
int LayeredTile::Layer::Load( std::ifstream& rInputStream )
{
   for (unsigned int j = 0; j < LayerHeight; ++j)
   {
      for (unsigned int i = 0; i < LayerWidth; ++i)
