...
public:
    ...
    wchar_t *GetProfileStringW(wchar_t *item, wchar_t *default_arg);
	DWORD   GetProfileInt(LPCTSTR item, DWORD default_arg);

	void WriteProfileString(LPCTSTR item, LPCTSTR value);
    void WriteProfileStringW(wchar_t *item, wchar_t *value);
	void WriteProfileInt(LPCTSTR item, DWORD value);
    void WriteProfileIntW(wchar_t *item, DWORD value);

    HKEY GetKey(){ return keyhandle; };

private:
	LPCTSTR subkey;
	HKEY    mainkey;
	HKEY    keyhandle;	
	TCHAR	returnstr[4096];
};
