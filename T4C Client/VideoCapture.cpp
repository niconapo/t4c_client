...
bool VideoCapture::TakeDesktopSnapshot(void)
{
   ...
   if (!GetDIBits(m_hDcBitmap, hBitmap, 0, (WORD) m_pBitmapInfoHeader->biHeight, m_pBitmapData, m_pBitmapInfo, DIB_RGB_COLORS)) 
   {
      lpDXDirectDraw->ReleaseDC(hDcWindow, NULL);
      return 0;
   }
...
bool VideoCapture::SaveToFile(const char* Path)
{
    ...
    do{
        sprintf_s( fileName,1024, "%s\\img%03u.bmp", Path, screenId );
...
//////////////////////////////////////////////////////////////////////////////////////////
{
	...
	try {
      ...
      ZeroMemory(&surfaceDesc, sizeof(surfaceDesc)); 
		...
		LPDIRECTDRAWSURFACE7 backBuffer = lpDXDirectDraw->PicklockBackBuffer( NULL, &surfaceDesc, DDLOCK_WAIT | DDLOCK_SURFACEMEMORYPTR, NULL );
		if( backBuffer == NULL ){
			return;
		}
...
//////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    for( i = frameList.begin(); i != frameList.end(); i++ ){
        ...
        // Search for a valid file name.
        do{
            sprintf_s( fileName,1024, "%s\\img%03u.bmp", directory.c_str(), screenId );
