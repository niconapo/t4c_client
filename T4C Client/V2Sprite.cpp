...
void BlendSSE(unsigned char* pPixelSrc, unsigned char* pPixelDst, unsigned char* pMask, int OffsetX, int OffsetY, int SizeX, int SizeY, int PitchS, int PitchD, int PitchAlpha);
void BlendMMX(unsigned char* pPixelSrc, unsigned char* pPixelDst, unsigned char* pMask, int OffsetX, int OffsetY, int SizeX, int SizeY, int PitchS, int PitchD, int PitchAlpha);
...
void CV2Sprite::LoadSpriteData(BYTE* pData, BYTE* pPal, int dwWidth, int dwHeight,int ForceTransparency)
{
    if(m_lpV2SpriteInfo )
       delete m_lpV2SpriteInfo;
    m_lpV2SpriteInfo = NULL;
    ...
    m_lpV2SpriteInfo->Header.ushTransparency = ForceTransparency;
...

    {
       ...
       DXDErrorHandle(lpDXDirectDraw->CreateSurfaceNM(&Surface, &m_lpV2SpriteInfo->Data.lpDDSurface, NULL), "Cheap Ass"); 
    }
    ...
    if(!lpbDest)
    {
       m_lpV2SpriteInfo->Data.lpDDSurface->Unlock(NULL);
       return;
    }
...
DWORD CV2Sprite::CreateSprite(LPCTSTR lpszID,DWORD dwRelativeIndex,int iPalID)
{
   ...
   if (m_strName.empty()) 
   {
      ...
      if (!dwIdxPos) 
      {
         ...
         char strTmp[512];
         sprintf_s(strTmp,512,"SPRITE ERROR1 [%s]\n",lpszID);
...
DWORD CV2Sprite::CreateSpriteByPal(LPCTSTR lpszID,DWORD dwRelativeIndex,LPBYTE iPalID,int iPalIndex)
{
   ...
   if (m_strName.empty()) 
   {
      ...
      if (!dwIdxPos)  
      {
         ...
         char strTmp[512];
         sprintf_s(strTmp,512,"SPRITE ERROR2 [%s]\n",lpszID);
         OutputDebugString(strTmp);
      }
   }
...
{
   return m_lpV2SpriteInfo->Header.ushTransparency;
}
 ...
 * Draw). It is used to make dynamic merging
 * 
...
void CV2Sprite::DrawSprite(int xPos,int yPos,RECT *Clip,RECT *OutBound,RECT *Wrap,
                     LPDIRECTDRAWSURFACE7 ToBlit,char FX,signed char Format,BOOL ParamDither,CV2Sprite *pSprMask)
{
   V2SPRITEFX spritefx;
   ZeroMemory(&spritefx, sizeof(V2SPRITEFX));
   ...
   DrawSpriteN(xPos, yPos, &spritefx,pSprMask); 
}
 ...
 *********************************************************************************/
HRESULT CV2Sprite::DrawSpriteN(LONG xPos,LONG yPos,LPV2SPRITEFX FX,CV2Sprite *pSprMask)
{
   if(pSprMask != NULL)
      int caca = 1;
	LPRECT lpRect;
   ...
   {
      case COMP_DD: // RAW
   	   hResult = DrawDDSprite(xPos, yPos, FX,FALSE,255,pSprMask);
 ...
 *
 //*********************************************************************************/
HRESULT CV2Sprite::DrawDDSprite(LONG xPos,LONG yPos,LPV2SPRITEFX FX,BOOL bSemiTrans,BYTE chAlphaLevel,CV2Sprite *pSprMask)
{
   ...
   LPDIRECTDRAWSURFACE7 lpSpriteSurface = m_lpV2SpriteInfo->Data.lpDDSurface;
   int Format = 0;
   LPDIRECTDRAWSURFACE7 ToBlit = FX->lpDirectDrawSurface;
   ...
   if (Wrap) 
   {
      ...
      else 
      {
         for (int i = 0; i < 4; i++) 
         {
            ...
            if(wTransOption !=12 && wTransOption !=14)
            ...
            else
            {
               ...
               Sprite.dwFlags           = DDSD_CAPS | DDSD_HEIGHT | DDSD_WIDTH;
               Sprite.ddsCaps.dwCaps    = DDSCAPS_SYSTEMMEMORY;
               Sprite.dwHeight          = Dest[i].bottom-Dest[i].top;
               Sprite.dwWidth           = Dest[i].right -Dest[i].left;
               ...
               if(Sprite.dwWidth >0 && Sprite.dwHeight>0)
               {
                  ...
                  destRectTmp.left   = 0;
                  destRectTmp.top    = 0;
                  destRectTmp.right  = Dest[i].right -Dest[i].left;
                  destRectTmp.bottom = Dest[i].bottom-Dest[i].top;
                  lpNewSurface->BltFast(0,0, ToBlit, &Dest[i], DDBLTFAST_WAIT);
                  ...
                  BYTE *pMask = NULL;
                  if(pSprMask)
                     pMask = pSprMask->m_lpV2SpriteInfo->Data.lpbChunck;
                  
                  TransAlphaMask2(m_lpV2SpriteInfo->Data.lpDDSurface,lpNewSurface,0,0,0,0, Source[i], ddColorKey.dwColorSpaceLowValue,16,pMask,
                                  pSprMask->m_lpV2SpriteInfo->Header.dwWidth,pSprMask->m_lpV2SpriteInfo->Header.dwHeight,Source[i].left,Source[i].top);
                                  
                  ToBlit->BltFast(Dest[i].left, Dest[i].top, lpNewSurface, &destRectTmp, DDBLTFAST_WAIT);
   ...
   else 
   {
      ...
      if (FX2 == 1) 
      {
         ...
         {
            LPDIRECTDRAWSURFACE7 lpNewSurface2;
            ...
            {
               DXDErrorHandle(lpDXDirectDraw->CreateSurfaceNM(&Sprite2, &lpNewSurface2, NULL), "Draw Sprite 5");
               RECT destRectTmp;
               destRectTmp.left   = 0;
               destRectTmp.top    = 0;
               destRectTmp.right  = destRect.right -destRect.left;
               ...
               if(boOutline)
               {
                  ...
                  if( ToBlit->IsLost() == DD_OK )
                  {
                     ...
                     if(!ddsd.lpSurface || !ddsd2.lpSurface)
                     {
                     ...
                     for (j = sourceRect.top; j < sourceRect.bottom; j++) 
                     {
                        ...
                        {
                           if(i < sourceRect.right-2/* && i >sourceRect.left+1*/)
                           {
                           ...
                           else if(dwStat==1/* && i < sourceRect.right-1*/)
                           {
         ...
         else if(wTransOption == 9 && pSprMask)
         {
            LPDIRECTDRAWSURFACE7 lpNewSurface;
            DDSURFACEDESC2 Sprite;
         ...
         else if(wTransOption == 11)
         {
            ...
            if(Sprite2.dwWidth >0 && Sprite2.dwHeight>0)
            {
               ...
               TransAlphaGhost(lpNewSurface,lpNewSurface2,0,0,0,0, sourceRect, ddColorKey2.dwColorSpaceLowValue,16,pMask,128);
               ToBlit->BltFast(destRect.left, destRect.top, lpNewSurface2, &destRectTmp, DDBLTFAST_WAIT);
         ...
         else if(wTransOption == 12)
         {
            ...
            if(Sprite.dwWidth >0 && Sprite.dwHeight>0)
            {
               ...
               {
                  if(boOutline)
                     TransAlphaMaskOutline(m_lpV2SpriteInfo->Data.lpDDSurface,lpNewSurface,0,0,0,0, sourceRect, ddColorKey.dwColorSpaceLowValue,16,pMask,boOutline,FX->dwOutlineColor,true);	   
                  else
                     TransAlphaMaskMMX(m_lpV2SpriteInfo->Data.lpDDSurface,lpNewSurface,sourceRect,16,pMask,true);
         ...
         else if(wTransOption == 13)
         {
            ...
            if(Sprite.dwWidth >0 && Sprite.dwHeight>0)
            {
               ...
               //TransAlphaMaskLower(m_lpV2SpriteInfo->Data.lpDDSurface,lpNewSurface,0,0,0,0, sourceRect, ddColorKey.dwColorSpaceLowValue,16,pMask,boOutline,FX->dwOutlineColor,true);	   
         ...
         else if(wTransOption == 14)
         {
            ...
            if(Sprite.dwWidth >0 && Sprite.dwHeight>0)
            {
               ...
               try
               {
                  TransAlphaMaskOutline2(m_lpV2SpriteInfo->Data.lpDDSurface,lpNewSurface,0,0,0,0, sourceRect, ddColorKey.dwColorSpaceLowValue,16,pMask,boOutline,FX->dwOutlineColor,true);	   
            ...
            else 
            {
               ...
               else if(wTransOption == 4)
               {
                  ...
                  if(Sprite.dwWidth >0 && Sprite.dwHeight>0)
                  {
                     ...
                     TransAlphaGlow22(m_lpV2SpriteInfo->Data.lpDDSurface,lpNewSurface,0,0,0,0, sourceRect, ddColorKey.dwColorSpaceLowValue,16,0);
                     ToBlit->BltFast(destRect.left, destRect.top, lpNewSurface, &destRectTmp, DDBLTFAST_WAIT);
                     lpNewSurface->Release();
                  }
                  */
               ...
               else if(wTransOption == 9 && pSprMask)
               {
                  ...
                  if(Sprite.dwWidth >0 && Sprite.dwHeight>0)
                  {
                     ...
                     TransAlphaImprovedMask(m_lpV2SpriteInfo->Data.lpDDSurface,lpNewSurface,0,0,0,0, sourceRect, ddColorKey.dwColorSpaceLowValue,16,pSprMask);
                     ToBlit->BltFast(destRect.left, destRect.top, lpNewSurface, &destRectTmp, DDBLTFAST_WAIT);
               ...
               else if(wTransOption == 11)
               {
                  ...
                  if(Sprite.dwWidth >0 && Sprite.dwHeight>0)
                  {
                     ...
                     TransAlphaGhost(m_lpV2SpriteInfo->Data.lpDDSurface,lpNewSurface,0,0,0,0, sourceRect, ddColorKey.dwColorSpaceLowValue,16,pMask,128);
                     ToBlit->BltFast(destRect.left, destRect.top, lpNewSurface, &destRectTmp, DDBLTFAST_WAIT);
               ...
               else if(wTransOption == 12)
               {
                  ...
                  if(Sprite.dwWidth >0 && Sprite.dwHeight>0)
                  {
                     ...
                     {
                        if(boOutline)
                           TransAlphaMaskOutline(m_lpV2SpriteInfo->Data.lpDDSurface,lpNewSurface,0,0,0,0, sourceRect, ddColorKey.dwColorSpaceLowValue,16,pMask,boOutline,FX->dwOutlineColor,false);	   
                        else
                           TransAlphaMaskMMX(m_lpV2SpriteInfo->Data.lpDDSurface,lpNewSurface,sourceRect,16,pMask,false);
               ...
               else if(wTransOption == 14)
               {
                  ...
                  if(Sprite.dwWidth >0 && Sprite.dwHeight>0)
                  {
                     ...
                     try
                     {
                        TransAlphaMaskOutline2(m_lpV2SpriteInfo->Data.lpDDSurface,lpNewSurface,0,0,0,0, sourceRect, ddColorKey.dwColorSpaceLowValue,16,pMask,boOutline,FX->dwOutlineColor,false);	   
                        ToBlit->BltFast(destRect.left, destRect.top, lpNewSurface, &destRectTmp, DDBLTFAST_WAIT);
                     }
                     catch (...)
                     {
...
HRESULT CV2Sprite::DrawNCKNormalSprite(LONG xPos,LONG yPos,LPV2SPRITEFX FX)
{
   ...
   // Variable
   DDSURFACEDESC2 Surface;    // Dest Surface Desc.
   SizeMemory(&Surface);
   // Result.
   HRESULT hr; 
   BOOL    boExit = FALSE;
   ...
   LPDIRECTDRAWSURFACE7 lpDirectDrawSurface = lpDXDirectDraw->GetBackBufferDirect();
   // General FX
   BOOL boOutline = FALSE;
...
// TransAlphaImproved - 
//////////////////////////////////////////////////////////////////////////////////
HRESULT CV2Sprite::TransAlphaImproved(LPDIRECTDRAWSURFACE7 src, LPDIRECTDRAWSURFACE7 dest,LONG lDestX, LONG lDestY,LONG lDestW,LONG lDestH, RECT srcRect, WORD ALPHA, DWORD ColorKey, WORD BPP)
{
   int register i,j;
   int height,width;
   BYTE* lpSprite;
...
HRESULT CV2Sprite::TransAlphaImprovedMask(LPDIRECTDRAWSURFACE7 src, LPDIRECTDRAWSURFACE7 dest,LONG lDestX, LONG lDestY,LONG lDestW,LONG lDestH, RECT srcRect, DWORD ColorKey, WORD BPP,CV2Sprite *pSprMask)
{
   int register i,j;
   int height,width;
   BYTE* lpSprite;
   ...
   // Initialize the pointers to the upper left hand corner of surface
   lpSprite  = (BYTE*)srcDDSD.lpSurface;
   lpSpriteM = pSprMask->m_lpV2SpriteInfo->Data.lpbChunck;
   lpDest    = (BYTE*)destDDSD.lpSurface;

   if(!lpSprite || !lpDest || !lpSpriteM)
      return DD_OK;
   ...
   switch(BPP)
   {
      case 16:
         ...
         // Initialize the pointers to the first pixel in the rectangle
         lpSprite   += (srcRect.top * SpritePitch) + (srcRect.left * 2);
         lpSpriteM  += (srcRect.top * pSprMask->m_lpV2SpriteInfo->Header.dwWidth) + (srcRect.left);
         lpDest   += (lDestY * dPitch) + (lDestX * 2);
         ...
         do
         {
            ...
            do
            {
               ...
               if (sTemp != ColorKey)
               {
                  ...
                  *((WORD*)lpDest) = (WORD)((MaskAlphaV * (sb - db) >> 8) + db |
                                           ((MaskAlphaV * (sg - dg) >> 8) + dg) << 5 |
                                           ((MaskAlphaV * (sr - dr) >> 8) + dr) << 11  );
               }
...
HRESULT CV2Sprite::TransAlphaGlow(LPDIRECTDRAWSURFACE7 src, LPDIRECTDRAWSURFACE7 dest,LONG lDestX, LONG lDestY,LONG lDestW,LONG lDestH, RECT srcRect, DWORD ColorKey, WORD BPP,int dwDiv,DWORD ColorKey2)
{
   int register i,j;
   int height,width;
   BYTE* lpSprite;
            ...
            do
            {
               ...
               do
               {
                  ...
                  if (sTemp != ColorKey)
                  {
                     ...
                     */
                     sr =m_pLutAlphaR[(sTemp & RBM)];
                     sg =m_pLutAlphaG[(sTemp & GBM)];
                     sb =m_pLutAlphaB[(sTemp & BBM)];
                     
                     dr =m_pLutAlphaR[(dTemp & RBM)];
                     dg =m_pLutAlphaG[(dTemp & GBM)];
                     db =m_pLutAlphaB[(dTemp & BBM)];
...
HRESULT CV2Sprite::TransAlphaGlow22(LPDIRECTDRAWSURFACE7 src, LPDIRECTDRAWSURFACE7 dest,LONG lDestX, LONG lDestY,LONG lDestW,LONG lDestH, RECT srcRect, DWORD ColorKey, WORD BPP,int dwDiv,DWORD ColorKey2)
{
   int register i,j;
   int height,width;
   BYTE* lpSprite;
...
HRESULT CV2Sprite::TransAlphaGhost(LPDIRECTDRAWSURFACE7 src, LPDIRECTDRAWSURFACE7 dest,LONG lDestX, LONG lDestY,LONG lDestW,LONG lDestH, RECT srcRect, DWORD ColorKey, WORD BPP,BYTE*pAlphaImage,BYTE chAlpha)
{
   int register i,j;
   int height,width;
   BYTE* lpSprite;
   ...
   switch(BPP)
   {
      case 16:
         ...
         iAlphaPos+= (srcRect.top * width) + (srcRect.left);
         ...
         sbufA = (WORD)((SpritePitch/2) - width);
         ...
         do
         {
            ...
            do
            {
               ...
               {
                  if(pAlphaImage)
                     chAlphaOK = pAlphaImage[iAlphaPos];
                  ...
                  */
                  ...
                  bR = (WORD)(((dr * chAlphaOK)   + sr  *(255 - chAlphaOK) ) / 255);
                  bG = (WORD)(((dg * chAlphaOK)   + sg  *(255 - chAlphaOK) ) / 255);
                  bB = (WORD)(((db * chAlphaOK)   + sb  *(255 - chAlphaOK) ) / 255);
...
HRESULT CV2Sprite::TransAlphaMaskMMX(LPDIRECTDRAWSURFACE7 src, LPDIRECTDRAWSURFACE7 dest, RECT srcRect, WORD BPP,BYTE*pAlphaImage,bool bFlipH)
{
   DDSURFACEDESC2 srcDDSD, destDDSD;
   // Set height and width of SPRITE
   ...
   if(!srcDDSD.lpSurface || !destDDSD.lpSurface)
   {
      src->Unlock(NULL);
      dest->Unlock(NULL);
      return DD_OK;
   ...
   //si on est inverser on doit calculer l'offsetX pour aller lire la bonne portion de l'iamge...
   
   int dwOffsetX = 0;
   if(bFlipH)
   {
      if(width < (int)srcDDSD.dwWidth)
      {
         //on doit calculer les offset en X
         //dwOffsetX = srcDDSD.dwWidth - width;
         if( srcRect.left >0) //coter gauche
            dwOffsetX = srcRect.left*-1; 
         else
            dwOffsetX = srcDDSD.dwWidth - width; 
   ...
   if(Custom.gdwUSEMMX)
      BlendMMX((BYTE*)srcDDSD.lpSurface, (BYTE*)destDDSD.lpSurface,pAlphaImage, srcRect.left+dwOffsetX, srcRect.top, width, height, (WORD)srcDDSD.lPitch, (WORD)destDDSD.lPitch,(WORD)srcDDSD.lPitch/2);
   else
      BlendSSE((BYTE*)srcDDSD.lpSurface, (BYTE*)destDDSD.lpSurface,pAlphaImage, srcRect.left+dwOffsetX, srcRect.top, width, height, (WORD)srcDDSD.lPitch, (WORD)destDDSD.lPitch,(WORD)srcDDSD.lPitch/2);
...
HRESULT CV2Sprite::TransAlphaMaskOutline(LPDIRECTDRAWSURFACE7 src, LPDIRECTDRAWSURFACE7 dest,LONG lDestX, LONG lDestY,LONG lDestW,LONG lDestH, RECT srcRect, DWORD ColorKey, WORD BPP,BYTE*pAlphaImage,BOOL bOutline, DWORD dwOutColor, bool bFlipH)
{
	int register i,j;
	int height,width;
	BYTE* lpSprite;
   ...
   if(bFlipH)
   {
	...
	switch(BPP)
	{
		case 16:
         //#if 0
			// Initialize the pointers to the first pixel in the rectangle
			lpSprite += (srcRect.top * SpritePitch) + ((srcRect.left+dwOffsetX) * 2);
			lpDest   += (lDestY * dPitch) + (lDestX * 2);
			iAlphaPos+= (srcRect.top * (SpritePitch/2)) + ((srcRect.left+dwOffsetX));
			...
			do
			{
					...
					{
						{
							...
							{
                        bR = (WORD)(((dr * chAlphaOK)   + sr  *(m_pAlphaI[chAlphaOK]) ) >> 8);
                        bG = (WORD)(((dg * chAlphaOK)   + sg  *(m_pAlphaI[chAlphaOK]) ) >> 8);
                        bB = (WORD)(((db * chAlphaOK)   + sb  *(m_pAlphaI[chAlphaOK]) ) >> 8);
							}
               ...
               if(j < width-2  && j > 1)
					{
						if(pAlphaImage[iAlphaPos+1] < 0xC0  && pAlphaImage[iAlphaPos+2] < 0xC0 && dwStat == 0)
						{
							*((WORD*)lpDest) = (WORD)dwOutColor;
							dwStat = 1;
						}
						else if(pAlphaImage[iAlphaPos] >= 0xC0 && dwStat == 1)
						{
							*((WORD*)lpDest) = (WORD)dwOutColor;
							dwStat = 0;
						}
					} 
					else if(dwStat==1 && i > 1/* && j < width-1*/)
					{
						*((WORD*)(lpDest+1)) = (WORD)dwOutColor;
						dwStat = 0;
					}
              // #endif
...
HRESULT CV2Sprite::TransAlphaMaskOutline2(LPDIRECTDRAWSURFACE7 src, LPDIRECTDRAWSURFACE7 dest,LONG lDestX, LONG lDestY,LONG lDestW,LONG lDestH, RECT srcRect, DWORD ColorKey, WORD BPP,BYTE*pAlphaImage,BOOL bOutline, DWORD dwOutColor, bool bFlipH)
{
   int register i,j;
   int height,width;
   BYTE* lpSprite;
...
HRESULT CV2Sprite::TransAlphaMask2(LPDIRECTDRAWSURFACE7 src, LPDIRECTDRAWSURFACE7 dest,LONG lDestX, LONG lDestY,LONG lDestW,LONG lDestH, RECT srcRect, DWORD ColorKey, WORD BPP,BYTE*pAlphaImage,int iMW,int iMH,int iMOX,int iMOY)
{
   int register i,j;
   int height,width;
   BYTE* lpSprite;
         ...
         do
         {
            ...
            do
            {
               ...
               if (sTemp != ColorKey)
               {
                  ...
                  chAlphaOK = /*255- */pAlphaImage[iAlphaPos];
                  //chAlphaOK = 0;
...
// Return: HRESULT, 
//////////////////////////////////////////////////////////////////////////////////////////
HRESULT AnimV2Sprite::DrawSpriteN(LONG xPos,LONG yPos,LPV2SPRITEFX FX,CV2Sprite *pSprMask)
{
   bool bIncreaseMove;
   ...
   if( curFrame == 0 )
   {
      if(bIncreaseMove)
         curFrame++;
      return CV2Sprite::DrawSpriteN( xPos, yPos, FX ,m_pMainAlpha);
   }

   HRESULT r = frames[ curFrame - 1 ]->DrawSpriteN( xPos, yPos, FX ,framesA[curFrame-1]);

   if( running && bIncreaseMove)
      curFrame++;
...
// 
//////////////////////////////////////////////////////////////////////////////////////////
void AnimV2Sprite::DrawSprite(int xPos,int yPos,RECT *Clip,RECT *OutBound,RECT *Wrap,LPDIRECTDRAWSURFACE7 ToBlit,
                              char FX,signed char Format,BOOL ParamDither,CV2Sprite *pSprMask)
{
    if( curFrame > frames.size() )
        curFrame = 0;
    ...
    {
        curFrame++;
        CV2Sprite::DrawSprite( xPos, yPos, Clip, OutBound, Wrap, ToBlit, FX, Format, ParamDither,pSprMask );
    }

    frames[ curFrame - 1 ]->DrawSprite( xPos, yPos, Clip, OutBound, Wrap, ToBlit, FX, Format, ParamDither,pSprMask );
    
    if( running )
        curFrame++;
}

void AnimV2Sprite::CreateSpriteMain(LPCTSTR lpszID,LPCTSTR lpszIDA)
{
   CreateSprite(lpszID);

   if(lpszIDA)
   {
      m_pMainAlpha = new CV2Sprite();
      m_pMainAlpha->CreateSprite(lpszIDA);
   }
...
// 
//////////////////////////////////////////////////////////////////////////////////////////
void AnimV2Sprite::AddFrame(const char *frameId,const char *frameIdA)
{
    // Create the frame 
    CV2Sprite *sprite = new CV2Sprite;
    sprite->CreateSprite( frameId ); 
    frames.push_back( sprite ); // Add it to the frames.

    if(frameIdA)
    {
       CV2Sprite *spriteA = new CV2Sprite;
       spriteA->CreateSprite( frameIdA ); 
      framesA.push_back( spriteA ); // Add it to the frames.
    }
    else  
    {
       CV2Sprite *spriteA = NULL;
      framesA.push_back( spriteA ); // Add it to the frames.
    }
...
void BlendSSE(unsigned char* pPixelSrc, unsigned char* pPixelDst, unsigned char* pMask, int OffsetX, int OffsetY, int SizeX, int SizeY, int PitchS, int PitchD, int PitchAlpha)
{
    //unsigned char* pSrc   = (pPixelSrc);// + (OffsetY * PitchS) + (OffsetX*2));
    //unsigned char* pDst   = (pPixelDst);
    //unsigned char* pAlpha = (pMask);

    unsigned char* pSrc   = (pPixelSrc+(OffsetY * PitchS)+(OffsetX*2));
    unsigned char* pDst   = (pPixelDst);
    unsigned char* pAlpha = (pMask+(OffsetY*PitchAlpha)+OffsetX);

    WORD sbuf,dbuf,sbufA;
    sbuf  = (WORD)(PitchS - (2 * SizeX));
    dbuf  = (WORD)(PitchD - (2 * SizeX));
    sbufA = (WORD)((PitchAlpha) - SizeX);

   int Remainder = (SizeX & 0x07); 
   int Width8     = (SizeX & ~0x07) / 8;
//    Remainder = SizeX;
//    Width8 = 0;

   const unsigned mu1 = 0xf800f800;
   const unsigned mu2 = 0x07e007e0;
   const unsigned mu3 = 0x001f001f;

   const unsigned int ou1 = 0x001f001f;
   const unsigned int ou2 = 0x003f003f;
   const unsigned int ou3 = 0x001f001f;

   // Loading masks
   __m128i REDMASK   = _mm_set_epi32(mu1, mu1, mu1, mu1);
   __m128i GREENMASK = _mm_set_epi32(mu2, mu2, mu2, mu2);
   __m128i BLUEMASK  = _mm_set_epi32(mu3, mu3, mu3, mu3);

   __m128i OFFSETRED   = _mm_set_epi32(ou1, ou1, ou1, ou1);
   __m128i OFFSETGREEN = _mm_set_epi32(ou2, ou2, ou2, ou2);
   __m128i OFFSETBLUE  = _mm_set_epi32(ou3, ou3, ou3, ou3);
...
do_next_8pixels:
         ...
         // Load source
         // Load destination
         //movdqu      xmm0, byte ptr [esi]
         //movdqu      xmm3, byte ptr [edi]
         movdqu      xmm0, byte ptr [edi]
         movdqu      xmm3, byte ptr [esi]
         movdqu      xmm1, xmm0
         movdqu      xmm4, xmm3
         movdqu      xmm2, xmm0
         movdqu      xmm5, xmm3
         ...
         paddw       xmm0, OFFSETRED
         paddw       xmm1, OFFSETGREEN
         paddw       xmm2, OFFSETBLUE
         ...
         //
         // Advance to the next height pixels.
         //
         add			edi, 16
         add			esi, 16
         add         edx, 8
...
done_8pixels_blocks:
      ...
      for (int i = 0; i < Remainder; ++i)
      {
         ...
         //unsigned short dwSourceTemp = *( ( unsigned short* ) pSrc );
         unsigned short dwSourceTemp = *( ( unsigned short* ) pDst );
         ...
         //unsigned short dwTargetTemp = *( ( unsigned short* ) pDst );
         unsigned short dwTargetTemp = *( ( unsigned short* ) pSrc );

         // Extract the red channels.
         unsigned short dwTgtRed = ( dwTargetTemp >> 11 ) & 0x1f;
         unsigned short dwSrcRed = ( dwSourceTemp >> 11 ) & 0x1f;

         // Extract the green channels.
         unsigned short dwTgtGreen = ( dwTargetTemp >> 5 ) & 0x3f;
         unsigned short dwSrcGreen = ( dwSourceTemp >> 5 ) & 0x3f;

         // Extract the blue channels.
         unsigned short dwTgtBlue = dwTargetTemp & 0x1f;
         unsigned short dwSrcBlue = dwSourceTemp & 0x1f;

         // Write the destination pixel.
         *( ( unsigned short* ) pDst ) = ( unsigned short ) 
            ( ( ( ( *pAlpha * ( dwSrcRed - dwTgtRed ) + 31) >> 8 ) + dwTgtRed ) << 11 |
            ( ( ( *pAlpha * ( dwSrcGreen - dwTgtGreen ) + 63) >> 8 ) + dwTgtGreen ) << 5 |
            ( ( ( *pAlpha * ( dwSrcBlue - dwTgtBlue ) + 31) >> 8 ) + dwTgtBlue ) );
...
void BlendMMX(unsigned char* pPixelSrc, unsigned char* pPixelDst, unsigned char* pMask, int OffsetX, int OffsetY, int SizeX, int SizeY, int PitchS, int PitchD, int PitchAlpha)
{
   ...
   int Width8     = (SizeX & ~0x03) / 4;
   //    Remainder = SizeX;
   //    Width8 = 0;
   
   const unsigned int mu1 = 0xf800f800;
   const unsigned int mu2 = 0x07e007e0;
   const unsigned int mu3 = 0x001f001f;
   ...
   // Loading masks
   unsigned int REDMASK[2]   = { mu1, mu1 };
   unsigned int GREENMASK[2] = { mu2, mu2 };
   unsigned int BLUEMASK[2]  = { mu3, mu3 };
   
   unsigned int OFFSETRED[2]   = { ou1, ou1 };
   unsigned int OFFSETGREEN[2] = { ou2, ou2 };
   unsigned int OFFSETBLUE[2]  = { ou3, ou3 };
...
do_next_8pixels:
         ...
         //       movq        mm3, byte ptr [edi]
         movq        mm0, byte ptr [edi]
            movq        mm3, byte ptr [esi]
            movq        mm1, mm0
            movq        mm4, mm3
            movq        mm2, mm0
            movq        mm5, mm3
            ...
            paddw       mm0, OFFSETRED
            paddw       mm1, OFFSETGREEN
            paddw       mm2, OFFSETBLUE
