...
// This function initialize the String
{
   ...
   strcpy_s(lpszString,strlen(lpszSource)+1, lpszSource);
}
...
// This function append a Sting to the existing String.
{
   ...
   if (lpszString) {
      ...
      strcpy_s(lpszTemp,strlen(lpszSource) + strlen(lpszString) + 1, lpszString);
      strcat_s(lpszTemp,strlen(lpszSource) + strlen(lpszString) + 1, lpszSource);
      delete lpszString;
      lpszString = lpszTemp;
   } else {
      lpszString = new TCHAR [strlen(lpszSource)+1];
...
// This function append a Byte to the existing String.
{
   ...
   if (lpszString) {
      ...
      strcpy_s(lpszTemp,strlen(lpszString) + 2, lpszString);
